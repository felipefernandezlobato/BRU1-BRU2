...
function MovimientosContent() {
  ...
  const buildQueryParams = useCallback(
    (offset: number) => {
      ...
      if (selectedDate) {
        ...
        const { from, to } = getMonthDateRange(calYear, calMonth);
    ...
    [selectedDate, calYear, calMonth, directionFilter]
  );
  ...
  const fetchMovements = useCallback(
    async (reset: boolean = true) => {
      try {
        ...
        const data = await apiFetch<Movement[]>(`/api/movements?${qs}`);
        ...
        const currentTotal = reset ? data.length : movements.length + data.length;
  ...
  const fetchCalendar = useCallback(async () => {
    try {
      ...
      params.set("month", String(calMonth + 1));
      if (directionFilter !== "all") params.set("direction", directionFilter);
      const data = await apiFetch<CalendarDay[]>(`/api/movements/calendar?${params}`);
      setCalendarData(data);
    } catch {
      // Silent fail — calendar decorations are non-critical
    }
  }, [calYear, calMonth, directionFilter]);
    ...
    <div className="p-4">
      ...
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm text-[#9CA3AF]">
          {loading
            ? "Cargando..."
            : `${totalCount} movimiento${totalCount !== 1 ? "s" : ""} — ${listLabel}`}
        </p>
        {selectedDate && (
          <button
            onClick={() => setSelectedDate(null)}
            className="text-xs text-[#861A22] font-medium px-2 py-1 rounded-lg active:bg-[#F8F0F1] touch-manipulation"
          >
            Ver todo el mes
          </button>
        )}
